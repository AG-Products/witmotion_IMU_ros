...
ROSWitmotionSensorController::ROSWitmotionSensorController()
  ...
  restart_service
      = node->create_service<std_srvs::srv::Empty>(_restart_service_name,
  ...
  imu_enable_velocities = node->get_parameter("imu_publisher.measurements.angular_velocity.enabled")
                              .get_parameter_value()
                              .get<bool>();
  ...
  load_parameter(imu_enable_orientation, "imu_publisher.measurements.orientation.covariance", -1.0,
  ...
  if (rtc_enable) {
    ...
    _rtc_topic
        = node->get_parameter("rtc_publisher.topic_name").get_parameter_value().get<std::string>();
...
void ROSWitmotionSensorController::imu_process(const witmotion_datapacket& packet) {
  ...
  static float ax, ay, az, t_acc;
  static float wx, wy, wz, t_gyro;
  static float roll, pitch, yaw, qx, qy, qz, qw;

  // Freshness counters.
  // These tell us whether accel and gyro are new since the last /imu publish.
  static uint64_t accel_seq = 0;
  static uint64_t gyro_seq = 0;
  static uint64_t last_published_accel_seq = 0;
  static uint64_t last_published_gyro_seq = 0;

  // Optional orientation state.
  static bool have_orientation_value = false;

  switch (static_cast<witmotion_packet_id>(packet.id_byte)) {
    case pidAcceleration:
      decode_accelerations(packet, ax, ay, az, t_acc);
      imu_have_accel = true;
      accel_seq++;
      break;

    case pidAngularVelocity:
      decode_angular_velocities(packet, wx, wy, wz, t_gyro);
      wx *= DEG2RAD;
      wy *= DEG2RAD;
      wz *= DEG2RAD;
    ...
    case pidAngles:
      decode_angles(packet, roll, pitch, yaw, t_acc);
      roll *= DEG2RAD;
      pitch *= DEG2RAD;
      yaw *= DEG2RAD;
  ...
  // Fill accel if enabled and available.
  ...
  // Fill gyro if enabled and available.
  ...
  // Fill orientation only if enabled and available.
  // But do NOT use orientation as the trigger for publishing.
  if (imu_enable_orientation && imu_have_orientation && have_orientation_value) {
  ...
  // ------------------------------------------------------------
  // Main fix:
  // Publish /imu only when BOTH accel and gyro are fresh.
  // Do not publish from angle/orientation packets alone.
  // ------------------------------------------------------------

  const bool accel_ready
      = (!imu_enable_accel) || (imu_enable_accel && accel_seq != last_published_accel_seq);

  const bool gyro_ready
      = (!imu_enable_velocities) || (imu_enable_velocities && gyro_seq != last_published_gyro_seq);

  // For FAST-LIO, we really want accel + gyro.
  // If either is enabled, wait for fresh values.
  if (!accel_ready || !gyro_ready) {
    return;
  }

  // Safety guard against duplicate burst publishes.
  static rclcpp::Time last_publish_time(0, 0, RCL_ROS_TIME);
  const rclcpp::Time now = rclcpp::Clock(RCL_ROS_TIME).now();

  if (last_publish_time.nanoseconds() != 0) {
    const double dt = (now - last_publish_time).seconds();

    // For 200 Hz, expected period is 0.005 s.
    // Anything below 1 ms is definitely duplicate/partial packet behaviour.
    if (dt < 0.001) {
      return;
    }
  }
  ...
  last_published_accel_seq = accel_seq;
  last_published_gyro_seq = gyro_seq;
}
...
void ROSWitmotionSensorController::gps_process(const witmotion_datapacket& packet) {
  if (gps_enable) {
    ...
    msg.position_covariance_type = have_accuracy
  ...
  if (is_active) {
    try {
      ...
      node->declare_parameter(param_name,
                              std::vector<std::string>({std::to_string(first_val), "0", "0", "0",
                                                        "0", "0", "0", "0", "0"}));
      auto tmp
          = node->get_parameter(param_name).get_parameter_value().get<std::vector<std::string>>();
